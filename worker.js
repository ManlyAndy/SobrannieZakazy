...
async function findDemand(code,auth){
  ...
  const dimensions=getAttr(d,DIMENSIONS_FIELD_NAME);
  ...
  return {found:true,id:d.id,name:d.name,agentName:d.agent?.name||"—",sum:d.sum?(d.sum/100).toFixed(2):"—",
    positionsCount:d.positions?.meta?.size??"—",places,dimensions,pickerName1:picker1,pickerName2:picker2,stateName,
...
async function handleCollect(req,auth){
  ...
  try{
    ...
    if(dimensions){
      ...
      else warning=`Поле "${DIMENSIONS_FIELD_NAME}" ещё не создано в МойСклад — габариты не сохранены, остальное сохранено.`;
    }
  ...
  return json({ok:true,name:d.name,places,dimensions:dimensions||null,pickerName1:picker1,pickerName2:picker2||null,stateName:STATUS_COLLECTED_NAME,warning});
}
